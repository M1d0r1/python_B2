...
def test_modify_contact(app, db, check_ui):
    ...
    with allure.step("Given list of contacts and the data for contact update"):
    ...
    with allure.step("When modifying the contact"):
        app.contact.modify_by_id(contact.id, contact)
    with allure.step("Then the new contact list is equal to the old one with the contact modified"):
...
def test_modify_contact_name(app, db, check_ui):
   ...
   with allure.step("Given a list of contacts"):
        old_contacts = db.get_contact_list()
   with allure.step("When modifying only the contact's name"):
        old_contact = random.choice(old_contacts)
        contact = old_contact
        contact.first_name = old_contact.first_name + RandomData.get_random_string()
        # Modify the contact
        app.contact.modify_by_id(contact.id, contact)
   with allure.step("Then the new list of contacts is equal to the old list with the modified contact"):
        new_contacts = db.get_contact_list()
        old_contacts.remove(old_contact)
        old_contacts.append(contact)
        assert sorted(old_contacts, key=Contact.id_or_max) == sorted(new_contacts, key=Contact.id_or_max)
