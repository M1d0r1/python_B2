...
class DbFixture:
    ...
    def get_contact_list(self):
        ...
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, middlename, lastname, nickname, company, title, address, home, mobile, work, fax, email, email2, email3, homepage, bday, bmonth, byear, aday, amonth, ayear, address2, phone2, notes from addressbook")
            ...
            for row in cursor:
                ...
                print(bmonth, ' ', amonth, ' ')
                print(FormatStrings.month_number(bmonth)," ", FormatStrings.month_number(amonth))
                try:
